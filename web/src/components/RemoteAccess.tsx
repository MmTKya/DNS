    ...
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-base-700/70 bg-base-850/40 p-4">
        ...
        <p className="mt-1 max-w-prose text-xs text-ink-muted">
          Nothing here can set this up: it is a rule on your router, and this
          node has no way to reach into it. If you do it anyway, forward to{" "}
          <span className="font-mono">
            {status.cloudflare?.service ?? "this node, port 8080"}
          </span>{" "}
          and switch on two-factor first — you are putting a box that can
          redirect every name in your house on the public internet, behind one
          password.
        </p>
      </div>
    </div>
...
function Cloudflare({
  ...
  const save = async (event: React.FormEvent) => {
    ...
    try {
      const saved = await api.saveCloudflare(
        tunnelID.trim(),
        credentials.trim(),
        hostname.trim(),
      );
      setResult({
        config: saved.config,
        path: saved.config_path,
        writeError: saved.write_error,
      });
      await onSaved();
    } catch (err) {
    ...
    <div className="rounded-xl border border-base-700/70 bg-base-850/40 p-4">
      <div className="flex flex-wrap items-baseline justify-between gap-3">
        ...
        <span className="text-xs text-ink-faint">
          {cf?.installed
            ? cf.version || "cloudflared installed"
            : "cloudflared is not installed here"}
        </span>
      </div>

      <p className="mt-1 max-w-prose text-xs text-ink-muted">
        No inbound port and your address stays hidden, but Cloudflare terminates
        TLS and can see the panel traffic. Pair it with Cloudflare Access so a
        password is not the only thing in front of your network.
      </p>

      {!cf?.installed && (
        <div className="mt-3 rounded-lg border border-base-700 bg-base-950/60 p-3">
          <p className="text-xs text-ink-muted">
            Install it first, then create a tunnel:
          </p>
...
cloudflared tunnel create seddns`}
          ...
          <p className="mt-2 text-xs text-ink-faint">
            The last command prints the tunnel id and the path of the
            credentials file. Those are the two values below.
          </p>
        </div>
      )}
      ...
      {result?.writeError && (
        <p className="mt-3 text-xs text-warn">
          Saved, but the file could not be written: {result.writeError}. Copy it
          from below instead.
        </p>
      )}
...
sudo systemctl status cloudflared`}
          ...
          <p className="mt-2 max-w-prose text-xs text-ink-faint">
            The node writes the file and stops there: it runs unprivileged and
            installing system services is not something a resolver should be
            able to do.
          </p>
        </div>
      )}
